...
test('G5 workbook retained E2 reading lessons carry source reading context', async () => {
  ...
  for (const descriptor of e2) {
    ...
    assert.ok(readingSection?.bullets?.[0]?.length > 40, `${descriptor.id} thiếu passage/context`);
  }
});
