...
import { validateTypingDraft } from '../src/features/admin/typing/typingContentDraft.js';
...
test('typing draft blocks empty targets before publish', () => {
  const lesson = { id: 'lesson', passThreshold: 80 };
  const result = validateTypingDraft(lesson, [{ id: 'w1', stage: 'word', vi: 'pizza', en: '', buildsFrom: [] }]);
  assert.ok(result.errors.some(error => error.includes('Typing item không hợp lệ')));
});
