...
test('word-bank UI remains structured metadata instead of parsing prompt text', () => {
  assert.match(enhancerSource,/g6-u1-wb-b5/); assert.match(enhancerSource,/g6-u1-wb-d1/); assert.match(enhancerSource,/sourceWordBank/);
...
test('app shell loads the dedicated non-interactive word-bank visual layer', () => {
  assert.match(indexHtml,/styles\/source-word-bank\.css/); assert.match(indexHtml,/src\/features\/drill\/sourceWordBankEnhancer\.js/);
