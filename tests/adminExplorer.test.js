...
test('Explorer tree nests Global Success 2, 5, 6 and 7 without changing Set ids', () => {
  ...
  const global2 = findAdminTreeNode(tree, 'global2');
  assert.deepEqual(global2.children.filter(node => node.type === 'folder').map(node => node.id), ['global2-unit6']);
  ...
  const g6Unit1 = findAdminTreeNode(tree, 'global6-unit1-writing-typing');
  assert.deepEqual(g6Unit1.children.filter(node => node.type === 'folder').map(node => node.id), [
...
test('Explorer breadcrumbs and recursive lesson counts match the hierarchy', () => {
  ...
  assert.deepEqual(folderBreadcrumbs(tree, 'global6-unit1-writing-final').map(item => item.label), ['Bài tập', 'Global Success 6', 'G6Unit1 Writing Typing', 'Final Writing Integration']);
...
test('Explorer search finds lessons by title, slug, Set id, Unit and activity type', () => {
  ...
  assert.ok(searchLessonDescriptors(sets, 'My school is + NAME').some(set => set.id === 'g6-u1-writing-s1-01'));
  ...
  assert.ok(searchLessonDescriptors(sets, 'Dịch hỏi–đáp').some(set => set.id === 'g2-u6-translation-09'));
...
test('Explorer type filters distinguish single-type lessons from mixed lessons', () => {
  ...
  const reading = sets.find(set => set.id === 'g5-u1-reading-01');
  ...
  assert.equal(lessonMatchesType(unit2, 'mix'), true);
  assert.equal(lessonMatchesType(unit2, 'classify'), false);
});
