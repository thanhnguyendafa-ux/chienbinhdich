    ...
    <main class="report-page">
      <nav class="report-toolbar shell" aria-label="Tác vụ báo cáo">
        <div class="report-toolbar-title">
          <strong>Báo cáo kết quả bài giao</strong>
      ...
      <article class="report-document shell ${submitted ? 'is-submitted' : 'is-abandoned'}">
        <header class="report-document-header">
          ...
          <strong class="result-stamp">${statusLabel(summary.status)}</strong>
        </header>
        ...
        ${metricSection('Thời gian chi tiết', [
          ...
          [
            ['Tổng thời gian', formatDuration(summary.durationMs)],
        ...
        <section class="process-note">
          ...
          <p>Đúng/Sai ở phần đầu báo cáo được tính theo lần trả lời đầu tiên của từng câu trong chuỗi chính. Retry và correction được giữ riêng ở phần chi tiết quá trình. Paste và phản hồi rất nhanh chỉ là dữ liệu quá trình, không tự động bị coi là gian lận.</p>
        </section>
...
function renderAssignmentHero({ session, set, summary }) {
  return `
    <section class="report-assignment-hero">
      <p class="report-kicker">BÁO CÁO KẾT QUẢ BÀI GIAO</p>
      <h1>${esc(set.title)}</h1>
      ...
      <div class="report-question-results" aria-label="Kết quả câu hỏi">
        ${questionResult('Tổng số câu', summary.totalItems)}
        ${questionResult('Đã làm', summary.attemptedItems)}
        ${questionResult('Đúng', summary.correctFirstTry, 'Lần đầu')}
        ${questionResult('Sai', summary.wrongFirstTry, 'Lần đầu')}
      </div>
    </section>`;
}

function keyResult(label, value, note, extraClass = '') {
  return `<div class="report-key-result ${extraClass}"><span>${esc(label)}</span><strong>${esc(value)}</strong><small>${esc(note)}</small></div>`;
}

function questionResult(label, value, note = '') {
  return `<div class="report-question-result"><span>${esc(label)}</span><strong>${esc(value)}</strong>${note ? `<small>${esc(note)}</small>` : ''}</div>`;
}

function statusLabel(status) {
  if (status === 'passed') return 'PASS ✓';
  if (status === 'abandoned') return 'BỎ CUỘC';
  return 'ĐANG LÀM';
}
