...
export const basicQuestionDefinitions = Object.freeze({
...
function bindTyping({ root, onSubmit, attemptStartedAt }) {
  ...
  root.querySelector('#question-form')?.addEventListener('submit', event => {
    ...
    onSubmit({ response, attemptMeta: attemptMeta(attemptStartedAt, tracker.inputMethod(), tracker.pasteDetected()) });
  });
...
function bindMcq({ root, onSubmit, attemptStartedAt }) {
  root.querySelectorAll('[data-choice-id]').forEach(button => button.addEventListener('click', () => {
    disableChoices(root);
    onSubmit({ response: button.dataset.choiceId, attemptMeta: attemptMeta(attemptStartedAt, 'choice', false) });
  }));
...
function bindTrueFalse({ root, onSubmit, attemptStartedAt }) {
  root.querySelectorAll('[data-boolean]').forEach(button => button.addEventListener('click', () => {
    disableChoices(root);
    onSubmit({ response: button.dataset.boolean === 'true', attemptMeta: attemptMeta(attemptStartedAt, 'choice', false) });
  }));
...
function bindSentenceOrder({ root, item, onSubmit, attemptStartedAt }) {
  ...
  submit?.addEventListener('click', () => {
    ...
    onSubmit({ response: selected.map(token => token.text), attemptMeta: attemptMeta(attemptStartedAt, 'tap', false) });
  });
