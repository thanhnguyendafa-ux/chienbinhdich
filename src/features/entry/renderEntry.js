    ...
    <main class="page page-centered entry-page">
      <section class="entry-card ${directSet ? 'direct-entry-card' : ''}">
        ...
        <form id="name-form" class="entry-form">
          ...
          <p class="input-note">${previewMode ? 'Chế độ Preview không gửi session lên Firebase.' : requiresTheory ? 'Nhập tên → đọc hết Lý thuyết → xác nhận → vào làm bài.' : directSet ? 'Nhập tên rồi bắt đầu ngay đúng bài thầy đã gửi.' : 'Tên và thời gian làm bài sẽ xuất hiện trong báo cáo cuối.'}</p>
          <button class="primary-btn" type="submit">${previewMode ? 'Bắt đầu Preview' : requiresTheory ? 'ĐỌC LÝ THUYẾT TRƯỚC →' : directSet ? 'Bắt đầu bài test' : 'Bắt đầu học'}</button>
        </form>
      </section>
    </main>`;
