...
let adminFlowPromise = null;
let drillEnhancersPromise = null;
let teachingUiPromise = null;
...
async function ensureDrillEnhancers() {
  drillEnhancersPromise ??= Promise.all([
    import('./features/drill/integrityWarningGate.js'),
    import('./features/drill/longPromptEnhancer.js')
  ]);
  return drillEnhancersPromise;
}

async function getTeachingUi() {
  teachingUiPromise ??= Promise.all([
    import('./features/admin/preview/teachingToolbar.js'),
    import('./features/admin/preview/teachingPanelEnhancer.js')
  ]).then(([toolbar]) => toolbar);
  return teachingUiPromise;
}
...
async function getAdminFlow() {
  adminFlowPromise ??= import('./features/admin/adminFlow.js').then(({ createAdminFlow }) => createAdminFlow({
    root,
    ...
    validateLesson,
    startStudentPreview: showAdminStudentPreview,
    renderLoading
  }));
  return adminFlowPromise;
}

async function showAdmin() {
  return (await getAdminFlow()).showAdmin();
}

async function navigateAdmin(params = {}) {
  return (await getAdminFlow()).navigateAdmin(params);
}
...
async function showDrill() {
  ...
  const [lesson, teachingUi] = await Promise.all([
    loadSessionHistoricalLesson(),
    previewMode ? getTeachingUi() : ensureDrillEnhancers().then(() => null)
  ]);
  if (previewMode) await ensureDrillEnhancers();
  ...
  renderDrill({
    ...
    onSubmit: ({ response, attemptMeta }) => {
      ...
      if (previewMode && teachingController) {
        ...
        teachingUi?.decorateTeachingSuccess(root);
        return;
      }
  ...
  if (previewMode && teachingController && teachingUi) attachTeachingMode(lesson, teachingUi);
}

function attachTeachingMode(lesson, teachingUi) {
  ...
  teachingUi.attachTeachingToolbar({
    root,
    state: teachingController?.getState(),
    onExit: exitTeachingMode,
    onPrevious: () => navigate(() => teachingController.previous()),
    onNext: () => navigate(() => teachingController.next()),
    onSkipSection: () => navigate(() => teachingController.skipSection()),
    onJumpToSection: sectionId => navigate(() => teachingController.jumpToSection(sectionId)),
    onJumpToQuestion: value => navigate(() => teachingController.jumpToQuestion(value)),
    onResetCurrent: async () => {
      feedback = null;
...
function exitTeachingMode() {
  ...
  void navigateAdmin({ inspect: activeSetId });
}
