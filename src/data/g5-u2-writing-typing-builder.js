...
function teachingFeedback(source) {
  return Object.freeze({
    ...
    theory: 'Dựng câu theo cold-start: NHÌN & CHÉP → RECALL → CHUNK → BUILD/BRIDGE → FINAL. Không thêm từ hoặc quy tắc chưa được chuẩn bị trong chính lesson.',
    example: source.targetSentence
  });
}

export function buildG5U2Lesson(key, source, steps) {
  if (!source) throw new Error(`Thiếu source cho G5 U2 Writing lesson ${key}`);
  if (!Array.isArray(steps) || steps.length === 0) throw new Error(`Lesson ${key} phải có ít nhất một scaffold step`);

  const items = steps.map((step, index) => {
    const id = `g5u2-wr-${key}-q${String(index + 1).padStart(2, '0')}`;
    const previousIds = steps.slice(0, index).map((_, previousIndex) => `g5u2-wr-${key}-q${String(previousIndex + 1).padStart(2, '0')}`);
    const item = {
      id,
...
export function buildG5U2LessonMap(sourceRecords, specs) {
  ...
  return Object.freeze(Object.fromEntries(Object.entries(specs).map(([key, spec]) => [
    key,
    buildG5U2Lesson(key, sourceById.get(spec.targetSentenceId), spec.steps)
  ])));
}
