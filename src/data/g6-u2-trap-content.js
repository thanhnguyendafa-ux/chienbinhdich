...
import { requiredTheoryForTrap } from './g6-u2-trap-prelesson-theory.js';
...
export function getG6U2TrapContent(key) {
  ...
  if (!cache.has(key)) {
    ...
    const sanitized = sanitizeG6U2TrapGrade4(presented);
    cache.set(key, Object.freeze({
      ...sanitized,
      preLessonTheory: requiredTheoryForTrap(source.trapCode)
    }));
  }
